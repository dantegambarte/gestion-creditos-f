...
describe('Regresión — Dropdowns con appendTo="body" (US-01, US-02, CL-09, CR-14, GA-02)', () => {
  ...
  describe('US-02 — Filtro Rol en /admin/users', () => {
    ...
    it('panel de Rol no queda cortado — se adjunta a body', () => {
      cy.get('[data-cy="admin-users-role-filter"]').click();
      cy.get('body > .p-dropdown-panel').should('be.visible');
      cy.get('body > .p-dropdown-panel .p-dropdown-item').should('have.length.gte', 1);
    });

    it('panel de Estado no queda cortado — se adjunta a body', () => {
  ...
  describe('US-01 — Dropdown Rol en modal Nuevo Usuario', () => {
    ...
    it('dropdown Rol dentro del modal se adjunta a body y es visible', () => {
      ...
      cy.get('[data-cy="admin-users-create-modal"]').should('be.visible');
      cy.get('[data-cy="admin-users-create-modal"] p-dropdown').click();
      // Panel debe estar en body, no dentro del dialog
      cy.get('body > .p-dropdown-panel').should('be.visible');
      cy.get('body > .p-dropdown-panel .p-dropdown-item').should('have.length.gte', 1);
    });
  });
  ...
  describe('CL-09 — Filtro riesgo en /admin/clients', () => {
    ...
    it('dropdown de filtro se adjunta a body y no queda cortado', () => {
      ...
      cy.get('body > .p-dropdown-panel .p-dropdown-item').first().click();
      cy.get('body > .p-dropdown-panel').should('not.exist');
    });
  });
