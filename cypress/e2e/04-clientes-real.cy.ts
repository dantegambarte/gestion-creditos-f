...
describe('Gestión de Clientes real — Admin', () => {
  ...
  const buildCustomerData = () => {
    ...
    return {
      ...
      apellido: 'Cliente',
      apellidoEditado: 'ClienteEditado',
      dni: `6${stamp}1`,
      telefonoInicial: `381${stamp}`,
      direccion: 'Calle Test 123',
      fullName: `Test${stampLetters} Cliente`,
      fullNameEditado: `Test${stampLetters} ClienteEditado`,
    };
  };
   ...
   */
  const ensureRoleSession = (
    role: 'ADMIN' | 'SELLER',
    destination: '/admin/clients' | '/seller/clients',
  ) => {
    ...
    cy.location('pathname', { timeout: 15000 }).should('eq', destination);
  };
  ...
  const getVisibleDialog = () =>
    cy.get('.p-dialog:visible', { timeout: 10000 }).first();
  ...
  const clickVisibleDialogButton = (label: string) => {
    cy.contains('.p-dialog:visible button.p-button', label, { timeout: 20000 })
      .should('be.visible')
      .and('not.be.disabled')
      .click();
  };
  ...
  it('CU03 principal: crea y edita cliente real con persistencia cruzada', () => {
    ...
    cy.loginReal('ADMIN', '/admin/clients');
    ensureRoleSession('ADMIN', '/admin/clients');
    waitClientsReady();

    cy.get('[data-cy="btn-nuevo-cliente"] button', { timeout: 15000 }).click({
      force: true,
    });
    getVisibleDialog()
      .contains('.p-dialog-title', 'Crear Cliente', { timeout: 10000 })
      .should('be.visible');
    getVisibleDialog()
      .find('input[formControlName="nombres"]')
      .should('be.visible');
    ...
    getVisibleDialog()
      ...
      .type(data.nombre);
    getVisibleDialog()
      .find('input[formControlName="apellidos"]')
      .clear()
      .type(data.apellido);
    getVisibleDialog()
      .find('input[formControlName="dni"]')
      .clear()
      .type(data.dni);
    getVisibleDialog()
      .find('input[formControlName="telefonoPrincipal"]')
      .clear()
      .type(data.telefonoInicial);
    getVisibleDialog()
    ...
    cy.contains('tbody tr', data.fullName, { timeout: 15000 }).should(
      'be.visible',
    );
    ...
    cy.contains('.p-dialog:visible .p-dialog-title', 'Editar Cliente', {
      timeout: 10000,
    }).should('be.visible');
    cy.get('.p-dialog:visible input[formControlName="nombre"]', {
      timeout: 5000,
    }).should('have.value', data.nombre);
    cy.get('.p-dialog:visible input[formControlName="apellido"]').should(
      'have.value',
      data.apellido,
    );
    ...
    cy.get('.p-dialog:visible input[formControlName="apellido"]')
      .clear()
      .type(data.apellidoEditado);

    clickVisibleDialogButton('Guardar Cambios');

    cy.contains('.p-toast-message', 'Modificación Exitosa.', {
      timeout: 15000,
    }).should('be.visible');
    cy.get('.p-dialog:visible').should('not.exist');

    cy.contains('tbody tr', data.fullNameEditado, { timeout: 15000 }).within(
      () => {
        cy.contains(data.telefonoInicial).should('be.visible');
      },
    );
  ...
  describe('Modal Editar Cliente — estructura', () => {
    ...
    it('el formulario de edición tiene campos nombre, apellido y phone', () => {
      getVisibleDialog()
        .find('input[formControlName="nombre"]')
        .should('exist');
      getVisibleDialog()
        .find('input[formControlName="apellido"]')
        .should('exist');
    ...
    it('el botón "Guardar Cambios" no está deshabilitado con el formulario pre-poblado', () => {
      cy.contains('.p-dialog:visible button.p-button', 'Guardar Cambios', {
        timeout: 5000,
  ...
  describe('Modal Crear Cliente — validaciones', () => {
    ...
    it('el botón "Crear Cliente" está deshabilitado con el formulario vacío', () => {
      cy.contains('.p-dialog:visible button.p-button', 'Crear Cliente', {
        timeout: 5000,
      }).should('be.disabled');
    });
...
describe('Gestión de Clientes real — Seller', () => {
  ...
  it('Seller ve la tabla de clientes sin botones de edición', () => {
    ...
    cy.contains('button', 'Nuevo cliente', { timeout: 20000 }).should(
      'be.visible',
    );
    cy.get('tbody tr', { timeout: 20000 }).should('have.length.gte', 1);
